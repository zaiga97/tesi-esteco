...
class Intersection(Env):
    def __init__(self, np_cars: np.array, pedestrian_df: pd.DataFrame, step_scale: float = .6):
        ...
        self.done = True
        # Action space = {dx, dy}
        self.action_space = spaces.Box(np.array([-1, -1]).astype(np.float32),
    ...
    def update_state(self) -> None:
        if self.flip:
            self.state = np.concatenate([self.agent_pos, self.target_pos, -self.np_cars[self.t][1], -self.np_cars[self.t][0]])
        else:
    ...
    def calculate_reward(self):
        ...
        if self.progress < -0.1:
            reward += -1
        # Speed term
        reward += -((np.linalg.norm(self.last_action) / self.step_scale) ** 4) * 1.5
        ...
        if self.done:
            if not self.t - self.it >= self.max_episode_steps:
                reward += 80
        return reward

    def check_if_done(self):
        return (np.linalg.norm(self.agent_pos - self.target_pos) < 1) or ((self.t - self.it) >= self.max_episode_steps)


def make_intersection() -> Intersection:
    np_cars = np.load('Data/Elaborated/np_cars.npy')
    crossing_ped_df = pd.read_pickle('Data/Elaborated/crossing_ped_df.pkl')

    return Intersection(np_cars, crossing_ped_df)
